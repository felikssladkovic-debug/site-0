...
Examples of such files, where Metadata headers MAY be omitted:
- files are considered human-oriented only
- files, generated by method utilities with purpose other than to be used by method itself (example: generated code)
- etc files that do not participate in the method engine
